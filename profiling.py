#!/usr/bin/env python3
"""
Profiling script for HRM model training.
...
class ProfileConfig(pydantic.BaseModel):
    ...
    # Hyperparams - Minimal for profiling
...
def create_dataloader(config: ProfileConfig, split: str, rank: int, world_size: int, **kwargs):
...
def create_model(config: ProfileConfig, train_metadata: PuzzleDatasetMetadata, world_size: int):
    model_cfg = dict(
...
def init_train_state(config: ProfileConfig, train_metadata: PuzzleDatasetMetadata, world_size: int):
...
def save_train_state(config: ProfileConfig, train_state: TrainState):
...
def compute_lr(base_lr: float, config: ProfileConfig, train_state: TrainState):
...
@line_profile
def train_batch(config: ProfileConfig, train_state: TrainState, batch: Any, global_batch_size: int, rank: int, world_size: int):
    train_state.step += 1
...
def load_synced_config(hydra_config: DictConfig, rank: int, world_size: int) -> ProfileConfig:
    objects = [None]
    if rank == 0:
        config = ProfileConfig(**hydra_config)  # type: ignore

        # Naming
        if config.project_name is None:
            config.project_name = f"{os.path.basename(config.data_path).capitalize()} ACT-torch-PROFILE"
        if config.run_name is None:
            config.run_name = f"{config.arch.name.split('@')[-1]} {coolname.generate_slug(2)} PROFILE"
        if config.checkpoint_path is None:
...
@hydra.main(config_path="config", config_name="cfg_profile", version_base=None)
